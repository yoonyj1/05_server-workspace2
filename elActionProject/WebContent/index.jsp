...
<body>
	...
	<!-- 
		* JSP를 이루는 구성인자
		 1. 스크립팅 원소: JSP에서 자바코드를 직접 기술할수 있게하는 기능
		 			   ex) 스크립틀릿, 표현식(출력식) 등등
         ...
         3. 액션태그(표준액션태그, 커스텀액션태그): xml 기술을 이용해서 기존의 jsp 문법을 확장하는 기술을 제공하는 태그
         	>> 표준액션 태그(Standard Action Tag): JSP 페이지에서 바로 사용 가능(별도의 연동 필요 없음)
         									   모든 태그명 앞에 jsp: 접두어 붙음
         	
         	>> 커스텀 액션 태그(Custom Action Tag): JSP 페이지에서 바로 사용 불가(별도의 라이브러리 연동 필요)
